...
export default function Sidebar({
  ...
  //const stableUserId = userId || localStorage.getItem("meeting_user_id");
  const stableUserId =
    userId ||
    (typeof window !== "undefined"
      ? localStorage.getItem("meeting_user_id")
      : null);
  ...
  const fetchNotesFromAPI = async () => {
    ...
    try {
      ...
      console.log("Fetched notes from API:", notesArray);
      const improveData = notesArray
        .filter((item) => item.disable === false)
        .map((item, index) => ({
          ...item,
          docId: item._id,
          globalIndex: index, // Add global index for tracking position in the list
        }));
  ...
  const updateNoteInAPI = async (docId, index, newText) => {
    try {
      ...
      console.log(
        "Updating note in API with docId:",
        docId,
        "index:",
        index,
        "newText:",
      ...
      const currentNotes = apiNotes.map((note) => note.Notes);
  ...
  const handleAddNewNote = async () => {
    ...
    const result = await saveNotesToAPI([newNoteText.trim()]); // ✅ sirf new note bhejo
  ...
  const handleSaveEditedMessage = async () => {
    ...
    // 2. Broadcast edit to all participants via LiveKit
    if (room && room.localParticipant) {
      ...
      const payload = encoder.encode(JSON.stringify(editData));
    ...
    setMessages((prev) =>
      prev.map((msg) =>
        msg.id === editingMessageId
          ? { ...msg, text: editingMessageText.trim() }
          : msg,
      ),
    );
                          ...
                          (
                            <div key={msg.id} className="flex flex-col group">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                {/* Edit/Delete - only show on your own messages */}
                                {msg.senderId === stableUserId && (
                              ...
                              {editingMessageId === msg.id ? (
                                <div className="bg-white/5 p-2 rounded-lg">
                                  <textarea
                                    ...
                                    className="w-full bg-white/5 text-sm text-white resize-y focus:outline-none rounded p-2"
                                    rows={2}
                                    autoFocus
                                  />
                    ...
                    <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-1">
                      {apiNotes.map((note) => (
                        <motion.div
                          ...
                          className="w-full min-h-[80px] bg-gradient-to-br from-white/10 to-white/5 rounded-xl p-3 border border-white/10 hover:border-white/20 transition-all duration-200 group"
                        >
                    ...
                    <div className="space-y-3 max-h-[50vh] overflow-y-auto scrollbar-hide pr-1">
                      {apiNotes?.map((note) =>
                        note?.Notes.split("\n")
                          ...
                          .map((line, index) => (
                            <motion.div
                              key={`${note._id}-${index}`} // ✅ UNIQUE KEY
                              initial={{ opacity: 0, scale: 0.95 }}
                              animate={{ opacity: 1, scale: 1 }}
                              exit={{ opacity: 0, scale: 0.95 }}
                              className="w-full min-h-[50px] bg-gradient-to-br from-white/10 to-white/5 rounded-xl p-2 border border-white/10 hover:border-white/20 transition-all duration-200 group"
                            >
                              {editingNoteId === note._id ? (
                                // ✏️ EDIT MODE (full note edit hoga)
                                <div>
                                  <textarea
                                    value={editText}
                                    onChange={(e) =>
